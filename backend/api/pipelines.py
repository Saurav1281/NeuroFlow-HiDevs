...
async def create_pipeline(
    ...
    async with pool.acquire() as conn:
        ...
        existing = await conn.fetchval(
            "SELECT MAX(version) FROM pipelines WHERE name = $1",
            config.name
        )
...
@handle_errors
async def get_pipeline(pipeline_id: uuid.UUID):
    """
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            SELECT p.*,
                   (SELECT AVG(faithfulness) FROM evaluations e JOIN pipeline_runs pr ON e.run_id = pr.id WHERE pr.pipeline_id = p.id) as avg_faithfulness,
                   (SELECT AVG(answer_relevance) FROM evaluations e JOIN pipeline_runs pr ON e.run_id = pr.id WHERE pr.pipeline_id = p.id) as avg_relevance,
                   (SELECT AVG(context_precision) FROM evaluations e JOIN pipeline_runs pr ON e.run_id = pr.id WHERE pr.pipeline_id = p.id) as avg_precision,
                   (SELECT AVG(context_recall) FROM evaluations e JOIN pipeline_runs pr ON e.run_id = pr.id WHERE pr.pipeline_id = p.id) as avg_recall
            FROM pipelines p
            WHERE p.id = $1
            """,
            pipeline_id
...
async def update_pipeline(
    ...
    async with pool.acquire() as conn:
        ...
        existing = await conn.fetchrow(
            "SELECT name, config, description FROM pipelines WHERE id = $1",
            pipeline_id
        )
        if not existing:
        ...
        try:
            full_config = PipelineConfig(name=existing["name"], description=existing["description"], **current_config)
        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Invalid config: {str(e)}")

        # 2. Get next version
        next_version = await conn.fetchval(
            "SELECT MAX(version) + 1 FROM pipelines WHERE name = $1",
            existing["name"]
        )
...
@handle_errors
async def get_pipeline_analytics(pipeline_id: uuid.UUID):
    """
    ...
    async with pool.acquire() as conn:
        ...
        return {
            ...
            "cost": {
                ...
                "avg_per_query": (stats["total_cost_usd"] / (await conn.fetchval("SELECT COUNT(*) FROM pipeline_runs WHERE pipeline_id = $1", pipeline_id) or 1)) or 0
            },
            "quality": {
...
@handle_errors
async def get_pipeline_suggestions(pipeline_id: uuid.UUID):
    """
