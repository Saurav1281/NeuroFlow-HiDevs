...
from fastapi import APIRouter, Body, File, HTTPException, Security, UploadFile
...
from backend.security.validators import validate_file, validate_url
...
async def upload_documents(
    ...
    current_user: Any = Security(  # noqa: ANN401
        get_current_user, 
        scopes=["ingest"]
    ),
) -> dict[str, Any]:
    ...
    with tracer.start_as_current_span("ingestion.process") as span:
        ...
        async with pool.acquire() as conn:
            for file in files:
                ...
                with tracer.start_as_current_span("ingestion.security_check"):
                    ...
                    if injection_result["prompt_injection_detected"]:
                        logger.warning(
                            f"Prompt injection detected in document {file.filename}: "
                            f"{injection_result['pattern']}"
                        )
                ...
                with tracer.start_as_current_span("ingestion.chunk"):
                    await conn.execute(
                        ...
                        INSERT INTO documents (
                            id, filename, source_type, content_hash, status, created_at
                        )
                ...
                doc_metadata = {
                    "prompt_injection_detected": injection_result["prompt_injection_detected"]
                }
...
async def ingest_url(
    ...
    current_user: Any = Security(get_current_user, scopes=["ingest"]),  # noqa: ANN401
) -> dict[str, Any]:
    """
...
async def search_similar_chunks(chunk_id: uuid.UUID, limit: int = 5) -> list[dict[str, Any]]:
    ...
    async with pool.acquire() as conn:
        ...
        target_embedding = await conn.fetchval(
            "SELECT embedding FROM chunks WHERE id = $1", chunk_id
        )
