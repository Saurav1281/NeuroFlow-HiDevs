...
async def run_pipeline_task(query: str, pipeline_id: uuid.UUID, llm_client: NeuroFlowClient, redis: Redis):
    """
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("SELECT config, version FROM pipelines WHERE id = $1", pipeline_id)
        if not row:
    ...
    eval_score = 0.0
    async with pool.acquire() as conn:
        eval_row = await conn.fetchrow("SELECT overall_score FROM evaluations WHERE run_id = $1", uuid.UUID(run_id) if run_id else None)
        if eval_row:
