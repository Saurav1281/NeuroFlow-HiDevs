...
class AnomalyObserver:
    ...
    def __init__(
        self, prometheus_url: str = "http://prometheus:9090", api_url: str = "http://api:8000"
    ) -> None:
    ...
    async def _check_alerts(self) -> None:
        async with httpx.AsyncClient() as client:
            ...
            for alert in alerts:
                if (
                    alert.get("labels", {}).get("alertname") == "QualityAnomalyDetected"
            ...
            for pipeline_id in current_anomalies:
                if pipeline_id not in self.active_alerts:
                    logger.info(
                        f"Anomaly detected for pipeline {pipeline_id}. Triggering suggestions..."
                    )
                    try:
