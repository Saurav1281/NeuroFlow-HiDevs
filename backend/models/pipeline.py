...
class RetrievalConfig(BaseModel):
    dense_k: int = Field(
        ..., description="Number of chunks to fetch from vector search", example=10
    )
    sparse_k: int = Field(
        ..., description="Number of chunks to fetch from keyword search", example=5
    )
    reranker: str = Field(
        ...,
        description="Reranker model type: cross-encoder, colbert, etc.",
        example="cross-encoder",
    )
    top_k_after_rerank: int = Field(
        ..., description="Final number of chunks passed to LLM", example=3
    )
    query_expansion: bool = Field(
        default=True, description="Whether to expand queries via LLM", example=True
    )
    metadata_filters_enabled: bool = Field(
        default=True, description="Filter vectors by properties", example=True
    )
...
class GenerationConfig(BaseModel):
    ...
    temperature: float = Field(
        default=0.2, ge=0.0, le=2.0, description="Sampling temp", example=0.2
    )
...
class EvaluationConfig(BaseModel):
    auto_evaluate: bool = Field(
        default=True, description="Run automated evaluation hooks", example=True
    )
    training_threshold: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Quality bar to use for training", example=0.85
    )
...
class PipelineConfig(BaseModel):
    ...
    name: str = Field(
        ..., description="Unique label for the pipeline.", example="production-rag-v1"
    )
    description: str | None = Field(
        None, description="Detailed explanation.", example="Standard doc query routing"
    )
