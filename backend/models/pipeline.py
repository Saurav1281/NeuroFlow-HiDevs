...
class IngestionConfig(BaseModel):
    chunking_strategy: str = Field(..., description="Strategy for chunking: hierarchical, semantic, etc.")
    ...
    extractors_enabled: List[str] = Field(..., description="List of file extractors: pdf, docx, etc.")
...
class GenerationConfig(BaseModel):
    model_routing: Dict[str, Any] = Field(..., description="Routing params: task_type, max_cost_per_call")
    max_context_tokens: int = Field(..., description="Cap on total context tokens")
    temperature: float = Field(default=0.2, ge=0.0, le=2.0)
    system_prompt_variant: str = Field(..., description="Named prompt variant: precise, creative, etc.")
