...
class NeuroFlowUser(HttpUser):
    ...
    def on_start(self) -> None:
        ...
        response = self.client.post(
            "/auth/token",
            json={"client_id": "neuroflow-client", "client_secret": "neuroflow-secret"},
        )
...
class AdminUser(NeuroFlowUser):
    ...
    @task
    def check_evaluations(self) -> None:
        if not self.token:
            return
        self.client.get(
            "/evaluations/stream", headers=self.headers
        )  # Usually it's a list, but we have stream
