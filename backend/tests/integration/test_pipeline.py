...
async def ingest_test_document(client, headers, file_path):
...
async def wait_for_status(client, headers, doc_id, target_status, timeout=60):
...
@pytest.mark.asyncio
async def test_full_rag_pipeline(client, auth_headers):
    ...
    query_res = await client.post("/query", json={
...
@pytest.mark.asyncio
async def test_deduplication(client, auth_headers):
    # 1. Upload first time
    doc_id1 = await ingest_test_document(client, auth_headers, "tests/fixtures/test_doc.pdf")
    ...
    with open("tests/fixtures/test_doc.pdf", "rb") as f:
        files = {"files": (os.path.basename("tests/fixtures/test_doc.pdf"), f, "application/pdf")}
...
@pytest.mark.asyncio
async def test_circuit_breaker(client, auth_headers):
    # Mock the LLM provider to return 500 errors 5 times
    # This requires mocking the NeuroFlowClient or the specific provider.
    with patch("backend.providers.openai_provider.OpenAIProvider.complete", side_effect=Exception("LLM Error")):
...
@pytest.mark.asyncio
async def test_prompt_injection(client, auth_headers):
...
@pytest.mark.asyncio
async def test_pipeline_comparison(client, auth_headers):
    # Create two pipelines or use existing ones (simulated)
    # Verify both return results
    response = await client.post("/compare/pipelines", json={
        "query": "Test comparison",
        "pipeline_ids": [str(uuid.uuid4()), str(uuid.uuid4())]
    }, headers=auth_headers)
...
@pytest.mark.asyncio
async def test_fine_tuning_extraction(client, auth_headers):
