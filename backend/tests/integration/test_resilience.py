...
async def test_redis_failure_graceful_degradation():
    ...
    """
    with patch("backend.resilience.rate_limiter.RateLimiter.sliding_window_check", side_effect=Exception("Redis Down")):
...
@pytest.mark.asyncio
async def test_worker_failure_status_check():
    """
