...
from backend.api.pipelines import create_pipeline, get_pipeline
...
async def test_pipeline_logic():
    ...
    try:
        pc = PipelineConfig(**valid_config)
    ...
    with patch("backend.api.compare.get_pool", return_value=mock_pool), \
        ...
        print(f"✅ Total execution time for parallel run was small (simulated).")
