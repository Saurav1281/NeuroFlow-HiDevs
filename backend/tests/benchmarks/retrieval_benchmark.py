...
class RetrievalBenchmark:
    ...
    async def run(self):
        ...
        overall_stats = {s: {"HitRate@5": 0, "HitRate@10": 0, "MRR@10": 0, "NDCG@10": 0, "latency": 0} for s in strategies}
        ...
        for entry in self.dataset:
            ...
            for k, v in m.items(): overall_stats["dense"][k] += v
            ...
            for k, v in m.items(): overall_stats["sparse"][k] += v
            
            # 3. Hybrid (RRF)
            start = time.time()
            hybrid_results = await self.retriever.retrieve(query, use_hyde=False) # Fusion of dense + sparse
            overall_stats["hybrid_rrf"]["latency"] += (time.time() - start)
            m = self.calculate_metrics(hybrid_results, gt_id)
            for k, v in m.items(): overall_stats["hybrid_rrf"][k] += v
            ...
            for k, v in m.items(): overall_stats["hybrid_reranked"][k] += v
        ...
        for s in strategies:
            st = overall_stats[s]
            print(f"| {s:15} | {st['HitRate@5']:.3f} | {st['HitRate@10']:.3f} | {st['MRR@10']:.3f} | {st['NDCG@10']:.3f} | {st['latency']:.3f} |")
        ...
        diff = (overall_stats["hybrid_reranked"]["MRR@10"] - overall_stats["dense"]["MRR@10"]) / (overall_stats["dense"]["MRR@10"] + 1e-9)
