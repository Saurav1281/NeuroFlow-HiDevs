...
PRIVATE_IP_REGEX = re.compile(
    r"^(10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.|127\.|localhost)"
)
...
def validate_query(text: str, max_length: int = 5000) -> str:
    ...
    if len(sanitized) > max_length:
        raise HTTPException(
            status_code=400, 
            detail=f"Query too long. Max length is {max_length} characters."
        )
    return sanitized
...
def validate_url(url: str):
    ...
    if PRIVATE_IP_REGEX.match(hostname):
        raise HTTPException(status_code=400, detail="Access to private IP ranges and localhost is forbidden (SSRF protection).")
    ...
    if HAS_MAGIC:
        try:
            ...
            if file_type not in allowed_types:
                # Special case: text/plain can be detected as other things sometimes, but be strict for PDF
                if "pdf" in file.filename.lower() and file_type != "application/pdf":
                    raise HTTPException(status_code=400, detail="File signature does not match PDF extension.")
